...
# All award type codes — contracts + grants + direct payments + loans + other
AWARD_TYPE_CODES = [
    "A", "B", "C", "D",          # Contracts (IDVs + Definitive)
    "02", "03", "04", "05",      # Grants
    "06", "07",                  # Direct Payments
    "08", "09",                  # Loans
    "10", "11",                  # Other Financial Assistance
]
...
ROR_CACHE_FILE = "ror_cache.json"


# ── ROR Lookup ──────────────────────────────────────────────────────────────
def load_ror_cache() -> dict:
    if os.path.exists(ROR_CACHE_FILE):
        with open(ROR_CACHE_FILE, "r") as f:
            return json.load(f)
    return {}


def save_ror_cache(cache: dict):
    with open(ROR_CACHE_FILE, "w") as f:
        json.dump(cache, f, indent=2)


@st.cache_data(ttl=86400, show_spinner=False)
def lookup_ror_ids(names: tuple) -> dict:
    """Look up ROR IDs for university names using the ROR API v2."""
    cache = load_ror_cache()

    missing = [n for n in names if n not in cache]
    if missing:
        progress = st.progress(0, text="Looking up ROR IDs...")
        for i, name in enumerate(missing):
            progress.progress(i / len(missing), text=f"ROR lookup: {name[:50]}...")
            try:
                resp = requests.get(
                ...
                if items:
                    # Take the top match
                    top = items[0]
                    ror_id = top.get("id", "")            # e.g. https://ror.org/0432jq872
                    ror_name = ""
                    for n_entry in top.get("names", []):
                        if "ror_display" in n_entry.get("types", []):
                            ror_name = n_entry.get("value", "")
                            break
                    if not ror_name:
                        ror_name = top.get("names", [{}])[0].get("value", "") if top.get("names") else ""
                    cache[name] = {"ror_id": ror_id, "ror_name": ror_name}
                else:
                    cache[name] = None
            except Exception:
                cache[name] = None
...
def fetch_recipients_for_fy(agency_name: str, fy: int, state_code: str = None, limit: int = 100) -> list:
    """Fetch award-level data for higher-ed recipients for one agency + FY,
    then aggregate obligations by recipient name.

    Uses /search/spending_by_award/ (matches USAspending Advanced Search)
    with toptier awarding agency filter and all award type codes.
    """
    ...
    # Collect all award-level rows, then aggregate by recipient
    ...
    while True:
        ...
        try:
            resp = requests.post(f"{API_BASE}/search/spending_by_award/", json=payload, timeout=60)
            resp.raise_for_status()
            data = resp.json()
        ...
        for award in results:
            name = award.get("Recipient Name") or "Unknown"
            amount = award.get("Award Amount") or 0
            recipient_totals[name] = recipient_totals.get(name, 0) + amount
    ...
    return [{"name": name, "amount": total} for name, total in recipient_totals.items()]
...
state_filter = st.sidebar.selectbox("University State (recipient location)", options=["All States"] + US_STATES, index=0)
...
if "df_pivot" in st.session_state:
    ...
    ror_results = lookup_ror_ids(tuple(university_names))
    ...
    for name in university_names:
        ...
        if coords:
            ...
            ror = ror_results.get(name)
            ror_id_short = ror["ror_id"].replace("https://ror.org/", "") if ror else "—"
            map_records.append({
                "University": name,
                "ROR ID": ror_id_short,
                "lat": coords["lat"],
    ...
    # Insert ROR ID as the first column
    ror_col = []
    for name in display_df.index:
        ror = ror_results.get(name)
        if ror:
            ror_col.append(ror["ror_id"].replace("https://ror.org/", ""))
        else:
            ror_col.append("—")
    display_df.insert(0, "ROR ID", ror_col)
    ...
    # ── Download ────────────────────────────────────────────────────────
    export_df = df_pivot.reset_index()
    # Add ROR IDs to export
    export_df.insert(1, "ROR ID", export_df["Recipient"].map(
        lambda n: ror_results.get(n, {}).get("ror_id", "").replace("https://ror.org/", "") if ror_results.get(n) else ""
    ))
